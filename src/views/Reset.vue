<template>
    <div class="wrap">
        ...
        <div style="padding: 10px;">
            <van-button class="my-btn" size="large" @click="reset">重置密码</van-button>
        </div>
...
export default {
    ...
    methods: {
        ...
        reset() {
            ...
            this.$http.post("/login/reset",this.ruleForm).then(result => {
                this.$toast.clear();
                let res = result.data;        
                if (res.code == 1) {
        ...
        sendSms(){
			...
			this.$toast.loading({mask: true,duration:0});
            that.$http.post("/login/sendsms1",data).then(result => {
				this.$toast.clear();
                let res = result.data;
                if (res.code == 1) {              
